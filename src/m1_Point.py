...
class Point(object):
    ...
    def __repr__(self):
        #Format subsititutes value for the curly brackets
    ...
    def move_to(self, newx, newy):
        ...
        self.timesmoved = self.timesmoved + 1
        self.distancesum = self.distancesum + math.sqrt((self.x -
                                                   self.initialx)**2 + (
            self.y - self.initioaly)**2)
    ...
    def get_distance_from(self, newpoint):
        distance = math.sqrt((newpoint.x - self.x)**2 + (newpoint.y -
                                                         self.y)**2)
        return distance

    def get_distance_from_start(self):
        distance = math.sqrt((self.x - self.originalx)**2 + (self.y -
                                                             self.originaly)**2)
        return distance
    ...
    def halfway_to(self, incoming):
        midpointx = ((incoming.x - self.x)//2) + self.x
        midpointy = ((incoming.y - self.y)//2) + self.y
